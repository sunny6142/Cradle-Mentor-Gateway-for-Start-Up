    ...
    public function index()
    {
        if(Auth::user()->roll == 2)
        {
            $feedback  = DB::table('mentors')
                        ...
                        ->select('mentor_feedback.*','day_and_time.*')
                        ->where('day_and_time.date','<',Carbon::now()->subDay() )
                        ->where(['mentors.admins' => Auth::user()->id, 'day_and_time.approval' => 1, 'qualitative' => null])
                        ->first();
            //dump($feedback);

            $pending_appoval  = DB::table('mentors')
                            ...
                            // ->select('professional_expertise.*','industry_vertical_experience.*','users.*', 'mentors.*' ,'start_up_info.*','mentoring_session_time.*','request_mentor.*')
                                ...
                                ->where(['mentors.admins' => Auth::user()->id, 'mentor_status' => 1])
                                ->count();

            $approved_startup  = DB::table('mentors')
                                ->leftJoin('request_mentor', 'mentors.request_mentor', '=', 'request_mentor.request_mentor_id')
                                ->leftJoin('users', 'request_mentor.user', '=', 'users.id')
                                ->leftJoin('start_up_info','start_up_info.user_id','=', 'users.id')
                                ->leftJoin('mentoring_session_time','request_mentor.mentoring_session_time','=','mentoring_session_time.id')
            ...
            $Accepted  = DB::table('mentors')
                            ...
                            // ->select('professional_expertise.*','industry_vertical_experience.*','users.*', 'mentors.*' ,'start_up_info.*','mentoring_session_time.*','request_mentor.*')
                                ...
                                ->where(['mentors.admins' => Auth::user()->id, 'user_status' => 2])
                                ->count();
            ...
            $startup  = DB::table('mentors')
                                ...
                                ->select('professional_expertise.*','industry_vertical_experience.*', 'mentors.*' ,'start_up_info.*','mentoring_session_time.*','request_mentor.*','users.*')
        ...
        //    return $this->mentor_startup_list();
          ...
          return view('mentor_Dashboard',compact('pending_appoval','approved_startup','Accepted','list','feedback'));
        }
        else
        
        $mentor= DB::table('admins')->where(['roll' => 2])->get();
        $mentor = $mentor->count();

        $users= DB::table('users')->get();
        $users = $users->count();

        $approval  = DB::table('request_mentor')
        ->leftJoin('professional_expertise', 'request_mentor.professional_expertise', '=', 'professional_expertise.id')
        ->leftJoin('industry_vertical_experience', 'request_mentor.industry_vertical_experience', '=', 'industry_vertical_experience.id')
        ->leftJoin('mentoring_session_time', 'request_mentor.mentoring_session_time', '=', 'mentoring_session_time.id')
        ...
        $mentor_list= DB::table('admins')
        ->where('roll', '=', 2)->paginate(5);

        $startup = DB::table('users')
        ->leftJoin('start_up_info', 'start_up_info.user_id', '=', 'users.id')
        ->where('roll', '=', 3)->paginate(25);
        // return view('admin',compact('mentor','users','approval'));

        return view('admin_Dashboard',compact('startup','mentor_list','mentor','users','approval'));
    }
...
//
    ...
    public function setMeetingDate($reqmid)
    {
        return view('setDateTime',compact('reqmid'));
    }
        ...
        DB::transaction(function() use ($request) {
         ...
         //    dump($starttime);
            ...
            $Startup = DB::table('request_mentor')
                                ...
                                ->leftJoin('professional_expertise', 'request_mentor.professional_expertise', '=', 'professional_expertise.id')
                                ->leftJoin('industry_vertical_experience', 'request_mentor.industry_vertical_experience', '=', 'industry_vertical_experience.id')
                                ->select('start_up_info.*','users.*')
                                
                             //  ->select('request_mentor.*')
                                ->where(['mentors.request_mentor' => $request->id])
                                ->first();
...
//dump($Startup);
            $msg = '<body style="padding:5px">Dear Madam/Sir,,
                                      <br><br><br> We are pleased to inform you that there is a mentoring session request from a startup that matches your professional expertise. 
                                      <br>Kindly find below the brief about it,<br>

                                      <br><br>
                                      
                                      Startup Name : '.$Startup->start_up_name.'.<br>
                                      Domain Expertise Required : '.$Startup->expertise .'.<br>
                                      Industry Expertise : '.$Startup->industry_experience .'<br>
                                      Problem : '.$Startup->industry_experience.'<br>
                                      Request you to login to our portal and initiate further action.<br>
                                      <a href="www.mentorgateway.com/dashboard" target="_blank"> mentorgateway </a> <br><br>
                                        Regard,<br>
                                      <a href="www.mentorgateway.com" target="_blank"> www.mentorgateway.com </a> <br>
                                      <a href="mailto:admin-mentorgateway@cradle-edii.in" target="_top"> admin-mentorgateway@cradle-edii.in </a> <br>
                                      <a href="tel:9825013774">+91 98250 13774</a><br>                
            ...
            $headers .= "Content-Type: text/html; charset=ISO-8859-1\r\n";
            mail($to, $subject, $msg, $headers);
            mail('admin-mentorgateway@cradle-edii.in', $subject, $msg, $headers );
            mail('sunny6142@gmail.com', $subject, $msg, $headers );
        ...
        if($validator->fails()){
           ...
           //   return response()->json($validator);
            ...
            $mentor = Admin::create($val);
                ...
                //send mail
                $msg = '<body style="padding:5px">Dear Madam/Sir,,
                                      <br><br><br> Thank you very much for associating with us.<br> 
                                      <br>We have successfully created your account on our portal.<br>
                                            Kindly find below your login credential for accessing the portal.
                                      <br>
                                      <a href="www.mentorgateway.com" target="_blank"> mentorgateway</a> .

                                      <br><br>
                                      User ID/Email ID  : '.$val['email'].'.<br>
                                      Password  : '.$pass_t.'.<br>
                                          <br>For further assistance, kindly drop an email to admin-mentorgateway@cradle-edii.in<br>
            ...
            //$mentor[0]->email
            $from = 'info@mentorgateway.com';
            $to = $val['email']; //info@mentorgateway.com
            $subject = "Your mentor account has been successfully created on Mentorgateway ";
            $headers = "From: " . strip_tags($from) . "\r\n";
            $headers .= "Reply-To: ". strip_tags($from) . "\r\n";
            $headers .= "MIME-Version: 1.0\r\n";
            $headers .= "Content-Type: text/html; charset=ISO-8859-1\r\n";
