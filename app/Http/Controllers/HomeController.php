...
class HomeController extends Controller
{
    ...
    {
       // session(['key' => 'value']);
      //  dump(session('key', 'default'));
        
            if(Auth::user()->active != 1 || Auth::user()->active == null) {
                $status = true;
                $msg = "A confirmation email has been sent to you. Click on the confirmation link in the email to activate your account";
        ...
        $Approval_required = DB::table('request_mentor')
        ...
        $accepted = DB::table('request_mentor')
                            ->leftJoin('mentors', 'mentors.request_mentor', '=', 'request_mentor.request_mentor_id')
                              
                                ->where(['request_mentor.user' => Auth::user()->id,  'mentors.user_status' => 2])
        ...
        $feedback  = DB::table('mentors')
                                ...
                                ->select('user_feedback.*','day_and_time.*')
                                ->where('day_and_time.date','<',Carbon::now()->subDay() )
                                ->where(['day_and_time.approval' => 1, 'qualitative' => null,'users.id'=> Auth::user()->id])
                                ->first();
        
         
        if(!$start_up_info->isEmpty() && $start_up_info[0]->logo != null)
        session(['image' => $start_up_info[0]->logo]);
       // dump(session('image', 'default'));
    
        if($start_up_info->isEmpty() || $start_up_info[0]->how_know_about_us == null)
        return view('home',compact('feedback','start_up_info','nature_of_startup','industry','categories','yn','ipr_val','qualification','start_up_stage','innovative_val','know_about_us'));
        else        
        return view('startup_Dashboard', compact('feedback','list','request_mentor','Approval_required','accepted'));
       // dump($start_up_info);                 
    ...
    public function Accept_Mentor_Post(Request $req)
    {
        ...
        DB::transaction(function() use ($req) {
          ...
          $mentor = DB::table('request_mentor')
            ...
            $to = $mentor[0]->email; //info@mentorgateway.com
            $subject = "Accepting Mentor Request";
...
//      ->leftJoin('start_up_info', 'start_up_info.user_id', '=', 'request_mentor.user')
    ...
    public function Request_Mentor_Post(Request $request)
    {
      //  dump($request->all());
        ...
        $validator = Validator::make($request->all(), [
            ...
            'problem' => 'required|min:20|max:250',
        ...
        if($validator->fails()){
          ...
          DB::transaction(function() use ($request) {
              ...
              $start_up_info  =  DB::table('start_up_info')
                                ...
                                $professional_expertise  =  DB::table('professional_expertise')
                                ->where('id', '=', $request->professional_expertise)
                                ->get();

                                $industry_vertical_experience  =  DB::table('industry_vertical_experience')
                                ->where('id', '=', $request->industry_vertical_experience)
                                ->get();
            //      dump($request_mentor);
            //      return;          
              $msg = '<body style="padding:5px">Dear Admin,,
                                      <br><br><br> There is a new mentoring session request from startup. 
                                      <br>Kindly login to portal for further action.<br><br>


                                      Startup Name : '.$start_up_info[0]->start_up_name.'.<br>
                                      Domain Expertise Required : '.$professional_expertise[0]->expertise.'.<br>
                                      Industry Expertise Required : '.$industry_vertical_experience[0]->industry_experience.'<br>
                                      <br>
                                      <a href="www.mentorgateway.com" target="_blank"> www.mentorgateway.com </a> <br>
                                      <a href="mailto:admin-mentorgateway@cradle-edii.in" target="_top"> admin-mentorgateway@cradle-edii.in </a> <br>
                                      <a href="tel:9825013774">+91 98250 13774</a><br>                
            ...
            //$mentor[0]->email
            $from = 'info@mentorgateway.com';
            $to = "admin-mentorgateway@cradle-edii.in";
            $subject = "Request for mentoring from ";
            $headers = "From: " . strip_tags($from) . "\r\n";
            $headers .= "Reply-To: ". strip_tags($from) . "\r\n";
            $headers .= "MIME-Version: 1.0\r\n";
            $headers .= "Content-Type: text/html; charset=ISO-8859-1\r\n";
    ...
    public function submitForm(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'no_of_founders' => 'required|numeric',
        ]);
   ...
   $arr = [];
    ...
    public function submitForm4(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            'about_your_startup' => 'required|min:20|max:250',
            'problem' => 'required|min:20|max:250',
            'solution' => 'required|min:20|max:250',
            'uniqueness' => 'required|min:20|max:250',
            'revenue_method' => 'required|min:20|max:250',
            'competitors' => 'required|min:20|max:250',
        ]);
