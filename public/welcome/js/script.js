(function($) {
	...
	if($('.side-nav-two').length){
		...
		//Show Sidebar Button
		$('.sidenav-bar .toggle-btn,.header-style-five .btn-box .nav-btn').on('click', function(e) {
			e.preventDefault();
			$('body').addClass('active-side-nav');
		});
		
		//Dropdown Button
		$('.side-nav-two .toggle-btn').on('click', function(e) {
			e.preventDefault();
			$('body').removeClass('active-side-nav');
		});
		
		$('.side-nav-two .side-nav .navigation li.dropdown').append('<div class="dropdown-btn"><span class="fa fa-angle-down"></span></div>');
		
		//Dropdown Button
		$('.side-nav-two .side-nav .navigation li.dropdown .dropdown-btn').on('click', function() {
			$(this).prev('ul').slideToggle(500);
		});
	}
